...
- If you change `ConfigToml` or nested config types, run `just write-config-schema` to update `codex-source/codex-rs/core/config.schema.json`.
- When working with MCP tool calls, prefer using `codex-source/codex-rs/codex-mcp/src/mcp_connection_manager.rs` to handle mutation of tools and tool calls. Aim to minimize the footprint of changes and leverage existing abstractions rather than plumbing code through multiple levels of function calls.
...
- Avoid large modules:
  ...
  - This rule applies especially to high-touch files that already attract unrelated changes, such
    as `codex-source/codex-rs/tui/src/app.rs`, `codex-source/codex-rs/tui/src/bottom_pane/chat_composer.rs`,
    `codex-source/codex-rs/tui/src/bottom_pane/footer.rs`, `codex-source/codex-rs/tui/src/chatwidget.rs`,
    `codex-source/codex-rs/tui/src/bottom_pane/mod.rs`, and similarly central orchestration modules.
...
Run `just fmt` automatically after you have finished making code changes anywhere in this repository; the root recipe targets `codex-source/codex-rs`. Do not ask for approval to run it. Additionally, run the tests:

1. Do not run `cargo test` directly. Use `just test` so test execution follows the repo defaults.
2. Run the test for the specific project that was changed. For example, if changes were made in `codex-source/codex-rs/tui`, run `just test -p codex-tui`.
...
Before finalizing a large change to `codex-source/codex-rs`, run `just fix -p <project>` to fix any linter issues in the code. Prefer scoping with `-p` to avoid slow workspace‑wide Clippy builds; only run `just fix` without `-p` if you changed shared crates. Do not re-run tests after running `fix` or `fmt`.

## The `codex-core` crate

Over time, the `codex-core` crate (defined in `codex-source/codex-rs/core/`) has become bloated because it is the largest crate, so it is often easier to add something new to `codex-core` rather than refactor out the library code you need so your new code neither takes a dependency on, nor contributes to the size of, `codex-core`.
...
See `codex-source/codex-rs/tui/styles.md`.
...
This repo uses snapshot tests (via `insta`), especially in `codex-source/codex-rs/tui`, to validate rendered output.
...
These guidelines apply to app-server protocol work in `codex-source/codex-rs`, especially:
