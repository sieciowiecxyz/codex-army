...
impl App {
    ...
    pub(super) fn refresh_rate_limits(
        ...
        let request_handle = app_server.request_handle();
        let app_event_tx = self.app_event_tx.clone();
        let status_account_display = self.chat_widget.status_account_display().cloned();
        let plan_type = self.chat_widget.current_plan_type();
        let has_chatgpt_account = self.chat_widget.has_chatgpt_account();
        tokio::spawn(async move {
