...
pub(crate) fn single_line_footer_layout(
    ...
    let default_line = left_side_line(footer_indicators, default_state, key_hints);
    ...
    let state_line = |state: LeftSideState| -> Line<'static> {
        if state == default_state {
            ...
            left_side_line(footer_indicators, state, key_hints)
        }
    };
    ...
    if !footer_indicators.is_empty() {
        ...
        let mode_only_width =
            left_side_line(footer_indicators, mode_only_state, key_hints).width() as u16;
        if !context_requires_cycle_hint
            && can_show_left_with_context(area, mode_only_width, context_width)
        {
            return (
...
fn footer_from_props_lines(
    ...
    match props.mode {
        ...
        FooterMode::ComposerEmpty => {
            ...
            vec![left_side_line(footer_indicators, state, key_hints)]
        }
        FooterMode::ShortcutOverlay => {
